...
export const Xcp20Table = async () => {
  const xcp20sWithMintPercentage = await Promise.all(
    Object.entries(xcp20s).map(async ([xcp20Name, xcp20Details]) => {
      ...
      const btcBurnt = (datum.satoshi_price * (soldCount / Number(datum.give_quantity))).toFixed(8);
      const percentMinted = ((soldCount / datum.escrow_quantity) * 100).toFixed(2);
      return { name: xcp20Name, btcBurnt, percentMinted };
    })
  );
  ...
  return (
    <div>
      ...
      <table className="min-w-full divide-y divide-slate-700 dark:divide-slate-200">
        <thead>
          <tr>
            ...
            <th className="dark:text-slate-300">BTC Burned</th>
            <th className="dark:text-slate-300">Percent Minted</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-800 dark:divide-slate-200">
          {xcp20sWithMintPercentage.map(({ btcBurnt, name, percentMinted }) => (
            <tr key={name}>
              <td>
      ...
      <p>{xcp20sWithMintPercentage.reduce((acc, curr) => acc + Number(curr.btcBurnt), 0)} BTC</p>
    </div>
  );
};
